		...
		for (let i = 0; i < names.length; i++) {
			...
			suffixes.map((suffixVariant) => {
				...
				if (Object.keys(happinessObject).length === currentCombination.length) {
					...
					const currentHappyness = calculateHappiness(
						currentCombination,
						happinessObject
					);
...
export const generateCharArray = (charA, charZ) => {
	const tempArr = [];
	let firstLetter = charA.charCodeAt(0);
	let lastLetter = charZ.charCodeAt(0);
	for (; firstLetter <= lastLetter; ++firstLetter) {
		tempArr.push(String.fromCharCode(firstLetter));
	}
	return tempArr;
};
